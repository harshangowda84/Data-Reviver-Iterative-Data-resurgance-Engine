...
namespace KFA.Disks {
    ...
    public class MasterBootRecord : PhysicalDiskSection {
        ...
        public MasterBootRecord(PhysicalDisk disk) {
            ...
            long filePtr;
            Win32.SetFilePointerEx(disk.Handle, 0, out filePtr, EMoveMethod.Begin);

            m_Data = new byte[MBR_SIZE];
            uint numBytesRead;
            bool readSuccess = Win32.ReadFile(disk.Handle, m_Data, MBR_SIZE, out numBytesRead, IntPtr.Zero);
            if (!readSuccess) {
                throw new Exception("Failed to load the Master Boot Record!");
            }
