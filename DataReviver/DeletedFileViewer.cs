...
namespace DataReviver {
	...
	public partial class DeletedFileViewer : UserControl {
		private const string EMPTY_FILTER_TEXT = "🔍 Search files...";
		private int _animationFrame = 0;
		private string[] _animationFrames = { "⏳", "⌛" }; // Simplified animation for better performance
		private int _lastProgressPercentage = -1; // Track last percentage to avoid unnecessary updates
		...
		public DeletedFileViewer(Scanner scanner) {
			...
			this.Resize += DeletedFileViewer_Resize;
			this.Load += DeletedFileViewer_Load;
				...
				if (!this.IsDisposed && this.IsHandleCreated) {
					this.Invoke(new Action(() => {
						try {
							...
							Console.WriteLine($"Error in scan started UI update: {ex.Message}");
						}
					}));
				}
			} catch (Exception exc) { 
				Console.WriteLine($"Error in state_ScanStarted: {exc.Message}"); 
			}
		}
					...
					this.Invoke(new Action(() => {
						try {
							foreach (ListViewItem item in _files) {
								try {
									var metadata = item.Tag as INodeMetadata;
									if (metadata != null) {
										var node = metadata.GetFileSystemNode();
										if (node != null) {
											item.SubItems[4].Text = node.Path ?? "";
										}
										item.SubItems[5].Text = _recoveryDescriptions.ContainsKey(metadata.ChanceOfRecovery) 
											? _recoveryDescriptions[metadata.ChanceOfRecovery] 
											: "Unknown";
										item.BackColor = _recoveryColors.ContainsKey(metadata.ChanceOfRecovery) 
											? _recoveryColors[metadata.ChanceOfRecovery] 
											: Color.White;
									}
								} catch (Exception ex) {
									// Skip problematic items
									Console.WriteLine($"Error processing file item: {ex.Message}");
								}
							}
							...
							Console.WriteLine($"Error in scan finished UI update: {ex.Message}");
						}
					}));
				}
				_scanning = false;
			} catch (Exception exc) { 
				Console.WriteLine($"Error in state_ScanFinished: {exc.Message}"); 
			}
		}
		...
		void state_ProgressUpdated(object sender, EventArgs ea) {
			try {
				if (_scanner != null && !this.IsDisposed && this.IsHandleCreated) {
					this.BeginInvoke(new Action(() => {
						try {
							SetProgress(_scanner.Progress);
						} catch (Exception ex) {
							Console.WriteLine($"Error in progress update: {ex.Message}");
						}
					}));
				}
			} catch (Exception exc) { 
				Console.WriteLine($"Error in state_ProgressUpdated: {exc.Message}"); 
			}
		}
		...
		public void SetScanButtonScanning() {
			bScan.Enabled = false;
			if (_scanner != null) {
				bScan.Text = $"⏳ Scanning {_scanner.DiskName}...";
			} else {
				bScan.Text = "⏳ Scanning...";
			}
			bScan.BackColor = Color.FromArgb(255, 152, 0); // Orange color for scanning
			progressBar.Show();
			lblProgress.Show();
		...
		public void SetScanButtonFinished() {
			...
			bScan.Enabled = false;
			if (_scanner != null) {
				bScan.Text = $"✅ {_scanner.DiskName} Scan Complete!";
			} else {
				bScan.Text = "✅ Scan Complete!";
			}
			bScan.BackColor = Color.FromArgb(52, 199, 89); // Green color for completed
			progressBar.Hide();
			lblProgress.Hide();
			bRestoreFiles.Show();
		}
		...
		private void bScan_MouseEnter(object sender, EventArgs e)
		{
			if (bScan.Enabled)
			{
				bScan.Text = "🚀 Start Advanced Scan";
			}
		}

		private void bScan_MouseLeave(object sender, EventArgs e)
		{
			if (bScan.Enabled)
			{
				bScan.Text = "🔍 Start Scan";
			}
		}
		...
		private void SetProgress(double progress) {
			try {
				...
				// Only update UI if percentage changed (reduce UI updates)
				...
				Console.WriteLine($"Error in SetProgress: {ex.Message}");
			}
		}
		...
		private void UpdateTimer_Tick(object sender, EventArgs e) {
			...
			if (fileCount > _files.Count) {
				// Process in larger batches to reduce UI updates
				int batchSize = Math.Min(50, fileCount - _files.Count);
				var items = MakeListItems(deletedFiles.GetRange(_files.Count, batchSize));
				_files.AddRange(items);
				
				// Suspend drawing during batch update
				fileView.BeginUpdate();
				try {
					var filteredItems = items.Where(FilterMatches).ToArray();
					if (filteredItems.Length > 0) {
						fileView.Items.AddRange(filteredItems);
					}
				}
				finally {
					fileView.EndUpdate();
		...
		private void animationTimer_Tick(object sender, EventArgs e) {
			// Simple, lightweight animation - just cycle the icon
			_animationFrame = (_animationFrame + 1) % _animationFrames.Length;
			string currentFrame = _animationFrames[_animationFrame];
			
			// Update only the scan button text, avoid heavy operations
			if (bScan.Text.Contains("Scanning") && _scanner != null) {
				bScan.Text = $"{currentFrame} Scanning {_scanner.DiskName}...";
			}
		}
		...
		/// <summary>
		/// Handles the Load event to ensure columns are sized properly when first displayed.
		/// </summary>
		private void DeletedFileViewer_Load(object sender, EventArgs e) {
			ResizeColumnsToFitWindow();
		}

		/// <summary>
		/// Handles the Resize event to automatically adjust column widths when the window is resized.
		/// </summary>
		private void DeletedFileViewer_Resize(object sender, EventArgs e) {
			ResizeColumnsToFitWindow();
		}

		/// <summary>
		/// Automatically resizes ListView columns to utilize the full available width.
		/// Maintains proportional widths while ensuring all columns fit within the visible area.
		/// </summary>
		private void ResizeColumnsToFitWindow() {
			if (fileView.Columns.Count == 0 || fileView.Width <= 0) return;

			// Account for scrollbar width and padding
			int availableWidth = fileView.ClientSize.Width - SystemInformation.VerticalScrollBarWidth - 10;
			if (availableWidth <= 0) return;

			// Define proportional weights for each column (these add up to 100%)
			float[] columnWeights = { 0.30f, 0.12f, 0.08f, 0.15f, 0.20f, 0.15f }; // Name, Type, Size, Modified, Path, Recovery

			// Calculate minimum widths to ensure readability
			int[] minWidths = { 150, 80, 60, 100, 120, 120 };

			// Calculate total minimum width required
			int totalMinWidth = minWidths.Sum();
			
			if (availableWidth < totalMinWidth) {
				// If window is too small, use minimum widths
				for (int i = 0; i < fileView.Columns.Count && i < minWidths.Length; i++) {
					fileView.Columns[i].Width = minWidths[i];
				}
			} else {
				// Calculate proportional widths
				for (int i = 0; i < fileView.Columns.Count && i < columnWeights.Length; i++) {
					int calculatedWidth = (int)(availableWidth * columnWeights[i]);
					int finalWidth = Math.Max(calculatedWidth, minWidths[i]);
					fileView.Columns[i].Width = finalWidth;
				}
			}
		}
	}
